...
import { addSupplier } from "@/app/redux/slices/productSlice";

const Suppliermodal = ({ setSupplierModal, product}) => {
    const dispatch = useDispatch();

    const [supplier, setSupplier] = useState({
        name: "",
        productName: product?.name || "",
        totalAmount: (product?.stock || 0) * (product?.price || 0), 
        paidAmount: "",
        company: "",
        email: "",
        phone: "",
    ...
    const saveSupplier = async () => {
        ...
        const result = await dispatch(addSupplier(payload));
        console.log(result)
        ...
        if (result.payload.success) {
            ...
            alert(`${payload.productName} is Aleready in your Supplier log`)
        }
    };
